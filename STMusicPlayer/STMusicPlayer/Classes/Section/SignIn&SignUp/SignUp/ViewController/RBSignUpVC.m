...
- (void)viewDidLoad
{
    ...
    RBTextView *nameTextView = ({
        RBTextView *view = [RBTextView textViewWithLeftWidth:leftWidth padding:15 image:[UIImage rbImageWithColor:[UIColor greenColor]] title:nil placeHolder:@"填写昵称"];
        [self.view addSubview:view];
        self.nameTextField = view.textField;
        view;
    });
    ...
    RBTextView *accountTextView = ({
        RBTextView *view = [RBTextView textViewWithLeftWidth:leftWidth padding:15 image:[UIImage rbImageWithColor:[UIColor greenColor]] title:nil placeHolder:@"填写手机号"];
        [self.view addSubview:view];
        self.accountTextField = view.textField;
        view;
    });
    [accountTextView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.mas_equalTo(nameTextView.mas_bottom).offset(padding/2);
        make.left.mas_equalTo(padding);
        make.right.mas_equalTo(-padding);
        make.height.mas_equalTo(height);
    }];
    ...
    RBTextView *passwordTextView = ({
        RBTextView *view = [RBTextView textViewWithLeftWidth:leftWidth padding:15 image:[UIImage rbImageWithColor:[UIColor greenColor]] title:nil placeHolder:@"填写密码"];
        [self.view addSubview:view];
...
- (void)signUpBtnTapped: (id)sender
{
    ...
    if (self.nameTextField.text.length && self.accountTextField.text.length && self.passwordTextField.text.length && self.verifyCodeTextField.text.length)
    {
